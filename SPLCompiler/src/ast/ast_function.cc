...
llvm::Value* ASTNode_FunctionHead::codeGen()
{
	...
	bool hasPara = parameters->children.empty();
...
llvm::Value* ASTNode_FunctionDecl::codeGen()
{
	...
	{
		auto retVar = allocInEntryBlock(fun, retName, fun->getReturnType());
	...
	for (auto& arg : fun->args())
	{
		...
		if (*isRefIter++)
			// ref arg, dereference the arg to get the correct ptr
			auto derefArg = IRGenBuilder->CreateLoad(&arg, arg.getName() + "_ptr");
		
		auto localVar = allocInEntryBlock(fun, std::string(arg.getName()), pArg->getType());
		IRGenBuilder->CreateStore(pArg, localVar);
	...
	if (body->codeGen())
	{
		...
		else IRGenBuilder->CreateRet(nullptr);
